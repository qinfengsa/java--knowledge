...
![MVC-work.png](E:/mycode/GitHub/java-knowledge/note/img/MVC-work.png)
...
在 Bean 实例化前后，我们可以做增强处理（前、后置处理），相当于bean实例化前后插入了方法 
...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	//调用BeanPostProcessor后置处理器->实例化对象之后
	@Override 
...
部分Aware接口是通过ApplicationContextAwareProcessor实现的，
...
在容器启动阶段，允许我们在容器实例化 Bean 之前对注册到该容器的 BeanDefinition 做出修改
